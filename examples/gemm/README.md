...
In this example, we work on a SGEMM kernel that requires a complete interface to `cuLaunchKernel` (e.g. grid size and size of shared memory), which is not provided by `clpy.ElementwiseKernel`.
...
The SGEMM kernel expects input and output arrays to be in Fortran contiguous memory layout, and this layout is enforced by `clpy.asfortranarray`.
...
This function takes a text of code and name of the kernel as input and returns `clpy.cuda.Function` object.
...
Back to `clpy.cuda.Function`, this object allows you to call the kernel with CUDA's `cuLaunchKernel` interface.
