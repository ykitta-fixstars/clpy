...
class TestUserkernel(unittest.TestCase):
    ...
    def test_manual_indexing(self, n=100):
        in1 = clpy.random.uniform(-1, 1, n).astype(clpy.float32)
        in2 = clpy.random.uniform(-1, 1, n).astype(clpy.float32)
        uesr_kernel_1 = clpy.ElementwiseKernel(
            'T x, T y',
            'T z',
            '''
                z = x + y;
        ...
        uesr_kernel_2 = clpy.ElementwiseKernel(
            'raw T x, raw T y',
            'raw T z',
            '''
                z[i] = x[i] + y[i];
