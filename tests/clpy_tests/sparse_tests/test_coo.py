...
class TestCooMatrix(unittest.TestCase):
    ...
    def test_row(self):
        ...
        testing.assert_array_equal(
            self.m.row, clpy.array([0, 0, 1, 2], self.dtype))

    def test_col(self):
        self.assertEqual(self.m.col.dtype, numpy.int32)
        testing.assert_array_equal(
            self.m.col, clpy.array([0, 1, 3, 2], self.dtype))

    def test_init_copy(self):
        n = clpy.sparse.coo_matrix(self.m)
        self.assertIsNot(n, self.m)
        clpy.testing.assert_array_equal(n.toarray(), self.m.toarray())

    def test_init_copy_other_sparse(self):
        n = clpy.sparse.coo_matrix(self.m.tocsr())
        clpy.testing.assert_array_equal(n.toarray(), self.m.toarray())
...
class TestCooMatrixInit(unittest.TestCase):
    ...
    def test_data_invalid(self):
        with self.assertRaises(ValueError):
            clpy.sparse.coo_matrix(
                ('invalid', (self.row(clpy), self.col(clpy))),
                shape=self.shape)
    ...
    def test_row_invalid(self):
        with self.assertRaises(ValueError):
            clpy.sparse.coo_matrix(
                (self.data(clpy), ('invalid', self.col(clpy))),
                shape=self.shape)
    ...
    def test_col_invalid(self):
        with self.assertRaises(ValueError):
            clpy.sparse.coo_matrix(
                (self.data(clpy), (self.row(clpy), 'invalid')),
                shape=self.shape)
    ...
    def test_unsupported_dtype(self):
        with self.assertRaises(ValueError):
            clpy.sparse.coo_matrix(
                (self.data(clpy), (self.row(clpy), self.col(clpy))),
...
class TestIsspmatrixCoo(unittest.TestCase):
    ...
    def test_coo(self):
        x = clpy.sparse.coo_matrix(
            (clpy.array([0], 'f'),
             (clpy.array([0], 'i'), clpy.array([0], 'i'))),
            shape=(1, 1), dtype='f')
        self.assertTrue(clpy.sparse.isspmatrix_coo(x))
    ...
    def test_csr(self):
        x = clpy.sparse.csr_matrix(
            (clpy.array([], 'f'),
             ...
             clpy.array([0], 'i')),
            shape=(0, 0), dtype='f')
        self.assertFalse(clpy.sparse.isspmatrix_coo(x))
