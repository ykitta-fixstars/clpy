...
class TestCsrMatrix(unittest.TestCase):
    ...
    def test_data(self):
        ...
        testing.assert_array_equal(
            self.m.data, clpy.array([0, 1, 2, 3], self.dtype))

    def test_indices(self):
        self.assertEqual(self.m.indices.dtype, numpy.int32)
        testing.assert_array_equal(
            self.m.indices, clpy.array([0, 1, 3, 2], self.dtype))

    def test_indptr(self):
        self.assertEqual(self.m.indptr.dtype, numpy.int32)
        testing.assert_array_equal(
            self.m.indptr, clpy.array([0, 2, 3, 4], self.dtype))

    def test_init_copy(self):
        n = clpy.sparse.csr_matrix(self.m)
    ...
    def test_init_copy_other_sparse(self):
        n = clpy.sparse.csr_matrix(self.m.tocsc())
        clpy.testing.assert_array_equal(n.data, self.m.data)
        clpy.testing.assert_array_equal(n.indices, self.m.indices)
        clpy.testing.assert_array_equal(n.indptr, self.m.indptr)
        self.assertEqual(n.shape, self.m.shape)
    ...
    def test_init_copy_other_scipy_sparse(self):
        m = _make(numpy, scipy.sparse, self.dtype)
        n = clpy.sparse.csr_matrix(m.tocsc())
        self.assertIsInstance(n.data, clpy.ndarray)
        self.assertIsInstance(n.indices, clpy.ndarray)
        self.assertIsInstance(n.indptr, clpy.ndarray)
        clpy.testing.assert_array_equal(n.data, m.data)
    ...
    def test_init_dense(self):
        ...
        clpy.testing.assert_array_equal(n.indices, [1, 3, 2])
        clpy.testing.assert_array_equal(n.indptr, [0, 2, 2, 3])
    ...
    def test_init_dense_empty(self):
        ...
        clpy.testing.assert_array_equal(n.indptr, [0, 0, 0, 0])
    ...
    def test_init_dense_one_dim(self):
        ...
        clpy.testing.assert_array_equal(n.indices, [1, 3])
        clpy.testing.assert_array_equal(n.indptr, [0, 2])
    ...
    def test_copy(self):
        n = self.m.copy()
        self.assertIsInstance(n, clpy.sparse.csr_matrix)
        self.assertIsNot(n, self.m)
    ...
    def test_csr(self):
        x = clpy.sparse.csr_matrix(
            (clpy.array([], 'f'),
             ...
             clpy.array([0], 'i')),
            shape=(0, 0), dtype='f')
        self.assertTrue(clpy.sparse.isspmatrix_csr(x))
